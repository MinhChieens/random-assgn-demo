...
const Appointment = () => {
  ...
  const handleActivityChange = (e) => {
    setValues({ ...value, activity: e.target.value });
    const availDocs = cachedDoctors.filter(
  ...
  const handleDoctorAndDateChange = (e) => {
    setValues({ ...value, [e.target.name]: e.target.value });
    if (value.date === "" || value.doctor === "") return;
    // possible bug when current availableDoctors is empty
  ...
  const getListDoctors = async () => {
    ...
    const doctorList = querySnapshot.docs.map((doc) => {
      return { ...doc.data().value, id: doc.id };
    });
    setCachedDoctors(doctorList);
  };
  useEffect(() => {
    getListDoctors();
  }, []);
      ...
      <ul className="w-full mx-auto  grid grid-cols-2 gap-2  items-stretch  p-5">
        ...
        <li className="flex flex-col gap-1">
          ...
          {availableDoctors.length === 0 ? (
            ...
            <select
              onChange={(e) => handleDoctorAndDateChange(e)}
              className="block w-full h-10 px-4 py-2 border border-black bg-transparent rounded-md shadow-sm outline-none opacity-80"
              type="text"
              name="doctor"
              id="doctor"
